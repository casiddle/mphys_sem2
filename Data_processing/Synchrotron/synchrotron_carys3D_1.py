...
for file_number in file_numbers:
    ...
    filename=f"Data_processing/Synchrotron/h5_files/v3d_synchrotron_{file_number}.h5" 
...
energy_ranges = [
    (f"124-{total_e_max} eV", 124, total_e_max), 
    ("3.1-124 eV", 3.1, 124), 
    (f"{total_e_min}-3.1 eV", total_e_min, 3.1)
]     
...
photon_counts["Total"] = (total_photon_count,100)
...
for label, e_min, e_max in energy_ranges:
    ...
    # Debugging: Check if arrays are empty
    if energies_integration.size == 0 or photons_integration.size == 0:
        print(f"Warning: No data in range {e_min}-{e_max} for {label}.")
        continue  # Skip this range to avoid errors
...
labels = [key for key in photon_counts if key != "Total"]  # Exclude "Total"
sizes = [photon_counts[key][1] for key in labels]  # Get percentages

# **Plot Pie Chart**
plt.figure(figsize=(7, 7))
colors = ["skyblue", "lightcoral", "lightgreen","magenta","green","teal"]  # Colors for categories
plt.pie(sizes, labels=labels, autopct='%1.1f%%', colors=colors, startangle=140, wedgeprops={'edgecolor': 'black'})
plt.title("Photon Distribution by Energy Range")
plt.show()
