...
def sum_over_theta_and_phi(synchrotron_array, theta_array, phi_array):
    photons_per_energy_per_phi = integrate.simpson(synchrotron_array, x=theta_array, axis=1)
    photons_per_energy = integrate.simpson(photons_per_energy_per_phi, x=phi_array, axis=0)
    return photons_per_energy

def sum_over_energy_and_phi(synchrotron_array, energy_array, phi_array):
    photons_per_theta_per_phi = integrate.simpson(synchrotron_array, x=energy_array, axis=2)
    photons_per_theta = integrate.simpson(photons_per_theta_per_phi, x=phi_array, axis=0)
...
def sum_photons(lower_boundary, upper_boundary, energy_array, photons_per_energy):
    ...
    return integrate.simpson(desired_photons,x=energy_array)
...
photons_per_energy = sum_over_theta_and_phi(photon_distribution_matrix, theta_array, phi_array)
photons_per_theta = sum_over_energy_and_phi(photon_distribution_matrix, energy_array, phi_array)
x_rays_per_energy = sum_over_theta_and_phi(x_ray_distribution_matrix, theta_array, phi_array)
x_rays_per_theta = sum_over_energy_and_phi(x_ray_distribution_matrix, energy_array, phi_array)
critical_photons_per_energy = sum_over_theta_and_phi(synchrotron_array, theta_array, phi_array)
...
critical_photon_number = sum_photons(0, 1e10, energy_array, critical_photons_per_energy)
...
print("Total no. of critical photons: ",critical_photon_number)
